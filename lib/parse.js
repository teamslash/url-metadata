...
module.exports = function (url, body, options) {
  ...
  if (!metadata.get('icon')) {
    let icons = []
    $('link').each(function (index, el) {
      if (el.attribs && el.attribs.rel === 'icon' && el.attribs.href) {
        icons.push({
          sizes: el.attribs.sizes,
          href: el.attribs.href
        })
      }
    })
    metadata.set({ 'icon': icons })
  }

  if (!metadata.get('apple-touch-icon')) {
    let icons = []
    $('link').each(function (index, el) {
      if (el.attribs && el.attribs.rel === 'apple-touch-icon' && el.attribs.href) {
        icons.push({
          sizes: el.attribs.sizes,
          href: el.attribs.href
        })
    ...
    metadata.set({ 'apple-touch-icon': icons })
  }
